...
| 2026-08-07 | **GO — review corrections** | Raw SQL is explicitly outside the built-in executable contract; Ray PostgreSQL parallel requests fail closed; Iceberg S3 precedence and empty-table schema behavior are tested; third-party open-only Providers remain a warned Ray-adapter exception and are never a Gateway fallback | @jiangxt2 |
| 2026-08-07 | **GO — bounded-source extension contract frozen** | Provider and Binding discovery are symmetric and versioned; Provider-declared projection/path semantics remove consumer source branches; TableScan and Binding constraints cover Hive catalog tables backed by Parquet, ORC, or Iceberg. New bounded sources inside the Ray/Daft contract may add plugins and tests but cannot change Training, Inference, or Graph call chains | @jiangxt2 |
| 2026-08-14 | **GO — native Lance write delegation** | Tributo owns only the WriteGateway control plane. Ray Lance writes use the stable `tributo.ray.lance` Binding and `lance_ray.write_lance(stream=False)`; Daft uses `DataFrame.write_lance`. The Tributo fragment/commit writer is removed, and a future `Dataset.write_lance` adoption is confined to the existing Ray Binding | @jiangxt2 |
| 2026-08-25 | **GO — Ray HiveServer2 vertical slice** | The built-in `tributo.hive` Provider and `tributo.ray.hive` Binding delegate structured projection reads to locked `ray-hive==1.0`. The real Hive 4.2.0 Data Ingestion Gate verifies an ORC-backed fixture on a Ray worker together with Iceberg-on-MinIO; this does not claim native ORC/HDFS, Daft Hive, raw SQL, or writes | @jiangxt2 |
