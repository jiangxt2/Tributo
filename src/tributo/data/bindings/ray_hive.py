"""Distributed Ray Data binding for HiveServer2 query sources."""
...
from tributo.data.hive import HiveDataConnector
...
class RayHiveBinding:
    ...
    def compile(self, request: BindingCompileRequest) -> BindingCompilation:
        with binding_stage("validate_capabilities"):
            plan = require_parameterized_query(request.plan, "hive")
    ...
    def _build(request: BindingCompileRequest, plan: SqlScan) -> _NativePlan:
        ...
        if not isinstance(sql, str) or not sql.strip():
            raise JobConfigurationError("Hive query text is unavailable")
        ...
        dataset = HiveDataConnector().read(
            host=options.get("host") or "localhost",
            port=options.get("port") or 10000,
            database=options.get("database") or "default",
            username=options.get("user") or "default",
            password=options.get("password"),
            auth=options.get("auth") or "NONE",
            sql=sql,
            params=options.get("params") or {},
            batch_size=options.get("batch_size")
            or request.read_options.batch_size
            or 10_000,
            shard_mode=options.get("shard_mode") or "auto",
            hash_column=options.get("hash_column"),
            hash_shards=options.get("hash_shards") or 64,
    ...
    def _wrap(
        ...
        return BindingCompilation(
            ...
            physical_splits=PhysicalSplitSummary(
                detail="Hive query shards are delegated to Ray Data"
            ),
            diagnostics=(
