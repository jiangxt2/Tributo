...
class BaseTrainer(ABC):
    ...
    pipeline.  Note: ``on_setup_start`` exceptions are propagated
    unconditionally so that a callback can abort training early; the base
    class does NOT catch them in ``run()``.  Callbacks that want to
    swallow their own errors (e.g. via a custom ``raise_on_error`` flag)
    must do so inside the callback implementation.
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        # can write export results into ``self._summary`` (the contract
        # documented on ``run()``) without triggering type-check errors.
