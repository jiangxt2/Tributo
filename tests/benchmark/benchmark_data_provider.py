"""Benchmark instance: Provider compatibility path vs ingestion Gateway.
...
Baseline (provider): the existing Ray-only compatibility adapter. Candidate
(gateway): explicit ``IngestionGateway`` with the Ray Data engine. Both read
the same fixed Parquet dataset generated by ``generate_data.py``.
...
def _measure_single(
    ...
    Each path returns a native Ray Dataset and is fully materialized here.
    """

    def load_provider():
        return load_ray_dataset_from_source({"type": "parquet", "path": path})

    def load_gateway():
        result = open_ingestion(
            IngestionRequest(
                source=ParquetSourceConfig(path=path), engine="tributo.ray_data"
            )
        )
        try:
    ...
    loader = load_provider if backend == "provider" else load_gateway
...
def main() -> None:
    ...
    gateway = _run_child("gateway", args.data, args.iterations, args.warmup)
    provider = _run_child("provider", args.data, args.iterations, args.warmup)

    print("\n| Path | mean (s) | stdev (s) | min (s) | max (s) | peak RSS (MB) |")
    print("|------|----------|-----------|---------|---------|---------------|")
    print(_fmt("provider (compatibility baseline)", provider))
    print(_fmt("gateway (explicit Ray Data)", gateway))
